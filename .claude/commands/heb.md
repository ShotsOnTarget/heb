...
learn and consolidate as a single uninterruptible flow:
...
pipes them to `heb consolidate`, and closes the session. If
compaction has removed earlier contracts from context, it recovers
them from session state using `heb session read`.
