...
func TestEntanglementGatedBySurpriseTouches(t *testing.T) {
	cfg := DefaultConfig()
	written := []MemoryDelta{
		{Body: "drone_stats derived_by type_lookup"},
	}
	c := LearnResult{
		CorrectionCount: 2,
		PeakIntensity:   0.6,
...
func TestEntanglementClampMin(t *testing.T) {
	...
	c := LearnResult{
		...
		Implementation:  Implementation{SurpriseTouches: []string{"abc_file"}},
	}
	out := buildEntanglementDeltas(written, c, cfg)
	if len(out) != 1 {
		t.Fatalf("got %d, want 1", len(out))
...
// Critical interaction test: a lesson reinforces a body that also matches
// a surprise_touch in a corrective session. The reinforcement and
// entanglement signals both hit the same body, emitted as TWO separate
// deltas (appended, not merged).
func TestEntanglementInteractionWithReinforcement(t *testing.T) {
	...
	// Confirm both deltas reference the same body.
	if written[0].Body != ent[0].Body {
		t.Errorf("deltas address different bodies: %q vs %q", written[0].Body, ent[0].Body)
	}
}
