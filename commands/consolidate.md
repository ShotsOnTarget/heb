...
Pipes contract:learn>consolidate JSON (from `/learn`) into `heb consolidate`. The CLI
...
A contract:learn>consolidate JSON object (the output of `/learn`).
...
no summarising. Stdout carries both the `CONSOLIDATE` display block and
the contract:consolidate>memory JSON result; stderr carries a one-line summary.
