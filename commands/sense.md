---
description: Heb contract:sense>recall — sense a developer prompt and turn it into a structured query object
...
`/sense` does the same. Extract salient content tokens. Classify only the one
...
Raw prompt (verbatim, do not modify):
...
that out from context when it starts working. `/sense` does not need to decide.
...
Then, immediately after, the machine-readable JSON block (no extra prose
before or after):
...
key for the entire pipeline run. Pass it forward to `/recall` and all
subsequent steps.
