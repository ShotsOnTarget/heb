...
2. **Predict** — form testable predictions about what will happen
   ...
   prediction with its source tuples so `/learn` can close the
   Hebbian loop after execution.

This is stage 3 of the `/heb` pipeline. It runs after `/recall` and
...
prediction that `/learn` will verify at session end.
...
`/learn` can replace the stale memory at session end.
...
Two JSON objects, supplied as `$ARGUMENTS` in this order:

```
$ARGUMENTS
```

The first is the contract:sense>recall JSON emitted by `/sense` — it carries
`session_id`, `intent`, `tokens`, and the raw prompt. The second is
the contract:recall>reflect JSON emitted by `/recall` — it carries the `memories`
...
`/reflect` was invoked without both contracts — first try reading
from session state:
...
seeing the code. After execution, `/learn` will compare each element
...
it. This is the link that closes the Hebbian loop — when `/learn`
...
Emit **two blocks**, in this order. No prose before, between, or
after.
...
- `action` on every conflict entry is always the literal string
  `"create_successor"`. `/learn` uses this to write the successor
...
The JSON block is the contract `/learn` reads at session end. When
`conflicts` is non-empty, `/learn` writes `create_successor` events
